...
function elmMakeEsm() {
  ...
  const iife = fs.readFileSync(elmOutputFile, 'utf8')

  const esm = iife.slice(
      iife.indexOf("function F("),
      iife.lastIndexOf(");}")
    ).replace('_Platform_export({', 'export const Elm = {')

  fs.writeFileSync(elmOutputFile, esm, 'utf8')
  return { code: Buffer.from(esm, 'utf8') }
}

const variants = {
  "`elm make --optimize` (for reference)": () => elmMakeEsm(),
  ...
  "Just esbuild (transform API)": (code) => {
    return esbuild.transformSync(code, {
      // bundle: true, // not supported
      minify: true,
      pure: pureFuncs,
      target: "es2020",
      format: "esm",
  ...
  "Just esbuild (build API)": () => {
    const filename = `elm_${Date.now()}.mjs`;
    esbuild.buildSync({
      entryPoints: [elmOutputFile],
      outfile: filename,
